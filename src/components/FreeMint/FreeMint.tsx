...
export function FreeMint() {
  ...
  const eligiblityActivePhase = eligibilityResult?.mintPhases.find((phase) => phase.isActive);
    ...
    <Card minW="xs" w={["100%", "430px"]} bgColor={"rgba(0,0,0,0.75)"}>
      <CardBody>
        <VStack mt="6" gap={4} alignItems={"center"}>
          ...
          {!mintConfigLoading && mintConfigResult && (
            <>
              <Heading size="md">
                Total minted: {mintConfigResult.totalMintedAcrossAllPhases ?? 0} / {mintConfigResult.maxTokenSupplyAcrossAllPhases}
              </Heading>
      ...
      <CardFooter display={"flex"} flexDirection={"column"}>
        {(!walletAddress || !provider) && (
          <Button variant="solid" colorScheme="blue" onClick={() => openWidget(WidgetType.CONNECT)}>
            Connect Wallet
          </Button>
        )}
        {walletAddress && provider && eligibilityResult && !eligibilityLoading && !mintId && (
          <Button variant="solid" colorScheme="blue" isDisabled={!eligiblityActivePhase || !eligiblityActivePhase.isAllowListed || mintLoading || Boolean(mintId)} onClick={mintButton}>
            Mint
          </Button>
        )}
